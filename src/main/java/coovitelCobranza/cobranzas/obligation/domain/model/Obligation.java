package coovitelCobranza.cobranzas.obligation.domain.model;
...
public class Obligation {
    ...
    /**
     * Updates mutable fields coming from a batch import.
     * Applies the rule: if delinquencyDays > 0 → EN_MORA, otherwise AL_DIA.
     * Optional fields (segment, product, agentCode) are overwritten only when
     * the incoming value is not null.
     *
     * @param totalBalance   new total balance of the obligation
     * @param delinquencyDays updated delinquency days (≥ 0)
     * @param dueDate        new due date
     * @param segment        client segment (nullable)
     * @param product        financial product (nullable)
     * @param agentCode      assigned agent code (nullable)
     */
    public void updateFromBatch(BigDecimal totalBalance,
                                int delinquencyDays,
                                LocalDate dueDate,
                                String segment,
                                String product,
                                String agentCode) {
        this.totalBalance    = Objects.requireNonNull(totalBalance, "totalBalance cannot be null");
        this.delinquencyDays = delinquencyDays;
        this.dueDate         = dueDate;
        ...
        if (agentCode != null) this.agentCode  = agentCode;
        this.updatedAt = LocalDateTime.now();
    }
