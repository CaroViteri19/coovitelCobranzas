...
import coovitelCobranza.cobranzas.cargamasiva.infrastructure.parser.TxtAsociadoParser;
 ...
 *   <li>Validación estructural por fila — {@link AsociadoImportValidationService}</li>
...
public class CargaMasivaApplicationService {
    ...
    private final TxtAsociadoParser               txtParser;
     ...
     * {@link CargaMasivaException}. Si pasa la validación, se procesan fila a fila
     ...
     * @throws CargaMasivaException si el archivo es inválido o la validación falla
     */
     ...
     */
    private Long upsertCliente(AsociadoRowDTO row, Map<String, Long> cacheClientes) {
     ...
     */
    private void upsertObligacion(AsociadoRowDTO row, Long clienteId) {
     ...
     * @throws CargaMasivaException si el archivo no puede ser leído o tiene formato inválido
     */
    private List<AsociadoRowDTO> parsearArchivo(MultipartFile file, String fileName) {
        try {
            String ext = extension(fileName);
            if ("txt".equals(ext)) {
                log.debug("[CARGA-MASIVA] Usando TxtAsociadoParser para '{}'", fileName);
                return txtParser.parse(file.getInputStream());
            }
            // Por defecto: CSV
            log.debug("[CARGA-MASIVA] Usando CsvAsociadoParser para '{}'", fileName);
            return csvParser.parse(file.getInputStream());
        } catch (IOException e) {
            log.error("[CARGA-MASIVA] Error leyendo '{}': {}", fileName, e.getMessage());
            throw new CargaMasivaException("No se pudo leer el archivo: " + e.getMessage(), fileName);
